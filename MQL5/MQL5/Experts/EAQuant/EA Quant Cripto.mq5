...
input int                  InpMaxSpreadPips        = 1600;            // Max spread (pips, 0=off)
input int                  InpMaxSlippage          = 1600;            // Max slippage (points)
//+------------------------------------------------------------------+
input group                "➜ Position Size"  
input enLotMode            InpLotMode              = lot_balance;     // Lote mode   
input double               InpLotValue             = 0.1;             // Lote value    
input double               InpMaxLot               = 2.00;            // Max lot  
input int                  InpMaxLeverage          = 10;              // Max levereage
input double               StopLossProcent         = 2.0;             // Fallback SL % (0=off)
input int                  TakeProfit              = 10000;           // Take profit (pips)  
//+------------------------------------------------------------------+
input group                "➜ Stop Loss"
input ENUM_SL_MODE         InpSLMode               = SL_ATR;          // Stop mode
input double               InpSLValue              = 4.0;             // Stop value (pips ou Kv)
input int                  InpATRStopsLen          = 10;              // ATRStops: Length
input int                  InpATRStopsPeriod       = 5;               // ATRStops: ATR period
//+------------------------------------------------------------------+
input group                "➜ Trailling Stop"  
input double               InpTraillingStep        = 4000.0;           // Trailing distance (pts) 
input double               InpTraliingStart        = 3000.0;            // Trailing start (pts)
//+------------------------------------------------------------------+
input group                "➜ Strategy"  
input double               InpTimeAnalise          = 1;              // Time (sec)  
input double               InpPipsStep             = 4000.0;         // Pip Step (pips) 
...
input int                  TimeStart               = 0;              // Time start  
input int                  TimeEnd                 = 21;             // Time end
input bool                 InpTradeOnFriday        = true;           // Operar nas Sextas?
input int                  InpFridayCloseHour      = 21;             // Friday Close (server hour, 0=off)
//+------------------------------------------------------------------+
input group                "➜ Prop Firm Limits"
input double               InpMaxDailyDD           = 4.0;            // Max Daily Drawdown % (0=off)
input double               InpMaxDD                = 8.0;            // Max Total Drawdown % (0=off)
...
//--- Detecta se o símbolo é do tipo Forex "clássico" (onde o conceito de pip fracionário existe)
...
//---
   ...
   //Print("can_trade_open: " + can_trade_open);
...
//+------------------------------------------------------------------+
//| Controle de entrada: máximo 1 trade por candle                   |
//+------------------------------------------------------------------+
bool CanTradeThisCandle()
{
   // Não permite nova tentativa enquanto aguarda confirmação
   if(sets.m_waiting_transaction)
      return false;

   datetime current_bar = iTime(_Symbol, _Period, 0);

   // Série ainda não disponível
   if(current_bar <= 0)
      return false;

   // Já abriu uma posição neste candle
   if(current_bar == sets.m_last_trade_bar_time)
      return false;
