...
public:
   ...
   SAssetProfile DetectAssetProfile(string symbol) { return BuildDefaultProfile(symbol); }
                            ...
                            double adx_p50, double adx_p75)
    {
       ...
       m_profile.adx_trend_threshold = adx_p75;
       m_profile.adx_range_threshold = adx_p50;
    }
   ...
   double CustomADX(string symbol, ENUM_TIMEFRAMES period, int adx_period, int shift = 1);
...
SAssetProfile CMacroRegimeEngine::BuildDefaultProfile(string symbol)
{
   ...
   p.adx_trend_threshold = 40.0;
   p.adx_range_threshold = 30.0;
...
//+------------------------------------------------------------------+
//| CustomADX — calcula ADX sem carregar indicador                    |
//| Usa Wilder's smoothing (mesmo método do MT4)                     |
//+------------------------------------------------------------------+
double CMacroRegimeEngine::CustomADX(string symbol, ENUM_TIMEFRAMES period, int adx_period, int shift)
{
   if(adx_period <= 0) return 0.0;
   int bars_needed = adx_period * 3;
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   int copied = CopyRates(symbol, period, shift, bars_needed, rates);
   if(copied < bars_needed) return 0.0;

   double tr_sum = 0, plus_dm_sum = 0, minus_dm_sum = 0;
   for(int i = 1; i < bars_needed; i++)
   {
      double high_diff = rates[i].high - rates[i-1].high;
      double low_diff  = rates[i-1].low - rates[i].low;
      ...
      {
         tr_sum = tr_sum - (tr_sum / adx_period) + tr;
         plus_dm_sum = plus_dm_sum - (plus_dm_sum / adx_period) + plus_dm;
         minus_dm_sum = minus_dm_sum - (minus_dm_sum / adx_period) + minus_dm;
      }
   }

   if(tr_sum == 0) return 0.0;
   double plus_di = 100.0 * plus_dm_sum / tr_sum;
   double minus_di = 100.0 * minus_dm_sum / tr_sum;
   double di_sum = plus_di + minus_di;
   if(di_sum == 0) return 0.0;

   double dx = 100.0 * MathAbs(plus_di - minus_di) / di_sum;

   double adx = dx;
   double k = 2.0 / (adx_period + 1);
   for(int i = 1; i < adx_period; i++)
   {
      int idx = (bars_needed - 1) - i;
      if(idx < 1) break;

      double h_d = rates[idx].high - rates[idx-1].high;
      double l_d = rates[idx-1].low - rates[idx].low;
      double p_dm = (h_d > l_d && h_d > 0) ? h_d : 0;
      double m_dm = (l_d > h_d && l_d > 0) ? l_d : 0;

      double t1 = rates[idx].high - rates[idx].low;
      double t2 = MathAbs(rates[idx].high - rates[idx-1].close);
      double t3 = MathAbs(rates[idx].low - rates[idx-1].close);
      double tr_val = MathMax(t1, MathMax(t2, t3));

      tr_sum = tr_sum - (tr_sum / adx_period) + tr_val;
      plus_dm_sum = plus_dm_sum - (plus_dm_sum / adx_period) + p_dm;
      minus_dm_sum = minus_dm_sum - (minus_dm_sum / adx_period) + m_dm;

      if(tr_sum == 0) continue;
      double pdi = 100.0 * plus_dm_sum / tr_sum;
      double mdi = 100.0 * minus_dm_sum / tr_sum;
      double ds = pdi + mdi;
      if(ds == 0) continue;
      dx = 100.0 * MathAbs(pdi - mdi) / ds;
      adx = adx * (1.0 - k) + dx * k;
   }
...
{
   int score = 0;
   // Base de Confiança (0 a 35 points)
   if(m_last_r2 > 0.80) score += 35;
   else if(m_last_r2 > 0.60) score += 28;
   else if(m_last_r2 > 0.50) score += 20;
   else if(m_last_r2 > 0.40) score += 10;

   // Força do Regime (0 a 25 points)
   double strength = GetStrength();
   score += (int)(strength * 25);

   // ADX DNA (0 a 20 points)
   double current_adx = CustomADX(Symbol(), m_timeframe, 14, 1);
   if(current_adx > m_profile.adx_trend_threshold)
      score += 20;
   else if(current_adx > m_profile.adx_range_threshold)
      score += 12;
   else
      score += 5;

   // Liquidez (0 a 10 points)
   int liq = GetLiquidityState();
   if(liq == 1) score += 10;
   else if(liq == 0) score += 5;
