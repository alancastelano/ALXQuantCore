...
class CLogger
  {
private:
   string            project;              // Name of project and log file
   string            file;
...
public:
   ...
   void              SetSetting(string project,string file_name,string default_category="",int cache_time_limit=0);                // Settings


   void              init(string project);                                                                                         // Initialization, open file for writing
   ...
   void              write(string msg,string category,uchar red,uchar green,uchar blue,string file="",int line=0);   // Generating message
                                                                                  // Flushing cache into file
...
void CLogger::init(void)
  {
   ...
   StringConcatenate(path,"log\\log_","ALXLog","_",file,"_",date.day,date.mon,date.year);    // Generate path and file name
...
void CLogger::write(string msg,string category,color colorOfMsg,string file="",int line=0)
  {
   ...
   if(file!="" && line!=0)
     {
      StringConcatenate(msg_log,category,":|:",red,",",green,",",blue,":|:",TimeToString(TimeCurrent(),TIME_SECONDS),"    ","file: ",file,"   line: ",line,"   ",msg);
     }
   else
     {
