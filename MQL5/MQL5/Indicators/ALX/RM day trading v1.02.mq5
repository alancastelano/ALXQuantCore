//+------------------------------------------------------------------+
//|                                      DayTrading_H1_NoRepaint.mq5|
//|                                  Copyright 2026, Gemini AI User  |
//|                          CORRIGIDO: Sem Repintura                |
//+------------------------------------------------------------------+
...
// ANTI-REPINTURA: controla a última barra já processada
...
int OnInit()
{
   ...
   // CORREÇÃO: ArraySetAsSeries em todos os buffers
   ...
   // CORREÇÃO: EMPTY_VALUE explícito para cada plot
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Day Trading H1 [NR]");
                ...
                const int &spread[])
{
   ...
   // --- Carga histórica completa (primeira execução) ---
   if(prev_calculated == 0)
   {
      ...
      // Inicializa todos os buffers com EMPTY_VALUE
      ...
      ArrayInitialize(G_ibuf_204, 0);
   }
   else
   {
      // ANTI-REPINTURA: só processa se uma NOVA barra fechou
      // Barra [1] = última barra fechada. Se já foi processada, sai.
      if(time[1] == G_last_bar_time)
         return(rates_total);

      // Nova barra fechou — processa apenas ela (i = 1)
      limit = 1;
   }
   ...
   {
      // ANTI-REPINTURA: nunca sobrescreve barra que já tem sinal
      if(G_ibuf_188[i] != EMPTY_VALUE || G_ibuf_192[i] != EMPTY_VALUE)
         continue;

      // Inicializa buffers da barra atual
      G_ibuf_188[i] = EMPTY_VALUE;
      G_ibuf_192[i] = EMPTY_VALUE;
      G_ibuf_196[i] = EMPTY_VALUE;
      G_ibuf_200[i] = EMPTY_VALUE;

      // Propaga estado de tendência
      G_ibuf_204[i] = G_ibuf_204[i + 1];

      // ATR para espaçamento de objetos
      double atr_val[1];
   ...
   // ANTI-REPINTURA: marca a barra 1 como processada
   if(rates_total > 1)
      G_last_bar_time = time[1];

   // --- Alertas: dispara apenas uma vez por barra fechada ---
          ...
          const datetime &time[], const double &high[], const double &low[], const double &close[])
{
   ...
   if(AnalysisEnabled && Ai_16 && Gi_300 > 0 && Gd_292 > 0.0)
   {
      ...
      if(G_ibuf_204[Ai_0] == 0.0) // Long anterior
      {
         ...
         {
            int pips = (int)(MathAbs(G_high_252 - Gd_292) / _Point / 10);
      ...
      else // Short anterior
      {
         ...
         {
            int pips = (int)(MathAbs(Gd_292 - G_low_260) / _Point / 10);
...
void Dashboard()
{
   ...
   if(total > 0)
   {
      ...
      ALXLabel("alx_dash_title",         col1, row1 - row_space, 12, "ALX RM Daytrade v1.00 [NR]",                                         clrBlack);
...
//+------------------------------------------------------------------+
//| iHighestIdx / iLowestIdx — retornam índice absoluto              |
//+------------------------------------------------------------------+
