...
void Dashboard()
{
   ...
   ALXLabel("title",    col1, row1,          10, "ALX RM Intraday v1.01"           ,                clrBlack);
   ALXLabel("sep1",     col1, row1 + rs,     8,  "────────────────────────────────",                clrDimGray);
   ALXLabel("win_lbl",  col1, row1 + rs*2,   8,  "Winrate:",                                        clrBlack);
   ALXLabel("win_val",  col2, row1 + rs*2,   8,  (string)win_pc + "% (" + (string)g_winCount + "/" + (string)(int)total + ")",  clrTeal);
   ALXLabel("loss_lbl", col1, row1 + rs*3,   8,  "Erros:",                                          clrBlack);
   ALXLabel("loss_val", col2, row1 + rs*3,   8,  (string)loss_pc + "% (" + (string)g_loseCount + "/" + (string)(int)total + ")", clrCrimson);
   ALXLabel("avg_lbl",  col1, row1 + rs*4,   8,  "Média Sinal:",                                    clrBlack);
   ALXLabel("avg_val",  col2, row1 + rs*4,   8,  (string)avg_pip + " pips",                         clrTeal);

   ALXLabel("sprd_lbl", col1, row1 + rs*5,   8,  "Spread:",                                         clrBlack);
   ALXLabel("sprd_val", col2, row1 + rs*5,   8,  (string)spread + " pips",                          clrTeal);

   ALXLabel("sep1",     col1, row1 + rs,     8,  "────────────────────────────────",                clrDimGray);

   ALXLabel("regime_lbl", col1, row1 + rs*6, 8,  "Regime:",                                         clrBlack);
   ALXLabel("regime_val", col2, row1 + rs*6, 8,  (string)qs_regime       ,                          clrTeal);
