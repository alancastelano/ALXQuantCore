...
#include <ALXQuantCore\Strategy\HurstDFA.mqh>      CHurstDFA       m_hurst;   
...
double H  = m_hurst.Get();
double R2 = m_hurst.GetConfidence();
string marketSummary  = m_hurst.GetMarketSummary(); 
...
int OnInit()
{
  EventSetTimer(30); // 5 minuto

  Print("🔍 Iniciando teste QuantCore...");
   if(GetQCFullStatus(Symbol(), qs))
   {
      Print("✅ SUCESSO!");
      qs_regime = "📊 Regime: " + qs.summary;
      qs_news   = "📰 Notícia: " + (qs.news_active ? qs.next_news_name + " (" + qs.next_news_impact + ") em " + IntegerToString(qs.next_news_minutes_away) + " min" : "Nenhuma");
      qs_session= "🏢 Sessões: LSE=" + (qs.lse_open ? "ABERTA" : "FECHADA") + " | NYSE=" + (qs.nyse_open ? "ABERTA" : "FECHADA");
   }
   else
   {
      Print("❌ Falha ao conectar com QuantCore API");
   }
...
void Dashboard()
{
   ...
   double total   = g_winCount + g_loseCount;
   double win_pc  = (total > 0) ? MathRound((g_winCount  / total) * 100) : 0;
   double loss_pc = (total > 0) ? MathRound((g_loseCount / total) * 100) : 0;
   double avg_pip = (g_signalCount > 0) ? MathCeil(g_totalPips / g_signalCount) : 0;
   int    spread  = (int)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);

   ALXLabel("title",    col1, row1,          10, "ALX RM Intraday v1.02" + " " + _Symbol + " " + EnumToString(_Period)   ,                clrWhite);
   ALXLabel("sep1",     col1, row1 + rs,     8,  "────────────────────────────────",                clrDimGray);
   ALXLabel("win_lbl",  col1, row1 + rs*2,   8,  "Winrate:",                                        clrWhite);
   ALXLabel("win_val",  col2, row1 + rs*2,   8,  (string)win_pc + "% (" + (string)g_winCount + "/" + (string)(int)total + ")",  clrTeal);
   ALXLabel("loss_lbl", col1, row1 + rs*3,   8,  "Erros:",                                          clrWhite);
   ALXLabel("loss_val", col2, row1 + rs*3,   8,  (string)loss_pc + "% (" + (string)g_loseCount + "/" + (string)(int)total + ")", clrCrimson);
   ALXLabel("avg_lbl",  col1, row1 + rs*4,   8,  "Média Sinal:",                                    clrWhite);
   ALXLabel("avg_val",  col2, row1 + rs*4,   8,  (string)avg_pip + " pips",                         clrTeal);

   ALXLabel("sprd_lbl", col1, row1 + rs*5,   8,  "Spread:",                                         clrWhite);
   ALXLabel("sprd_val", col2, row1 + rs*5,   8,  (string)spread + " pips",                          clrTeal);

   ALXLabel("sep1",     col1, row1 + rs*6,   8,  "────────────────────────────────",                clrDimGray);

   ALXLabel("regime_trend_lbl", col1, row1 + rs*7, 8,  "Trend:",                                    clrWhite);
   ALXLabel("regime_trend_val", col2, row1 + rs*7, 8,  (string)m_hurst.GetTrend() ,                 GetColorTrend());
   
   ALXLabel("regime_volat_lbl", col1, row1 + rs*8, 8,  "Volatility:",                               clrWhite);
   ALXLabel("regime_volat_val", col2, row1 + rs*8, 8,  (string)m_hurst.GetVolatility() ,            GetColorVolatility());
   
   ALXLabel("regime_moment_lbl", col1, row1 + rs*9, 8,  "Momentum:",                                clrWhite);
   ALXLabel("regime_moment_val", col2, row1 + rs*9, 8,  (string)m_hurst.GetMomentum() ,             GetColorMomentum());
   

}

//--- TREND SEPARADO
string GetColorTrend()
{
   if(R2 < 0.70) return(clrDimGray);   // SIDEWAYS 🟡 (Low Conf)
   if(H > 0.55)  return(clrTeal);      // BULLISH 🟢
   if(H < 0.45)  return(clrCrimson);   // BEARISH 🔴
         
   return(clrGray); // SIDEWAYS 🟡
}

//--- VOLATILITY SEPARADO
string GetColorVolatility()
{
   double fast_atr = CustomATR(_Symbol, _Period, 14,1);
   double slow_atr = CustomATR(_Symbol, _Period, 100,1);
            
   if(fast_atr > slow_atr * 1.2) return(clrTeal); // HIGH 🔴
   if(fast_atr < slow_atr * 0.8) return(clrGray); // LOW 🔵
         
   return(clrOrange); // NORMAL 🟢
}

//--- MOMENTUM SEPARADO
string GetColorMomentum()
{
   int mom_state = m_hurst.GetMomentumState();
         
   if(mom_state == 2)  return(clrTeal);      // STRONG 💪
   if(mom_state == -1) return(clrCrimson);   // WEAK 👎"
   if(mom_state == -2) return(clrOrange);    // REVERSAL ⚠
         
   return(clrOrangeRed);   // MODERATE 👍
}
...
void OnTimer()
{
   ...
   if(current_bar!= last_bar_hurst)
   {
      ...
      H  = m_hurst.Get();
      R2 = m_hurst.GetConfidence();
      marketSummary  = m_hurst.GetMarketSummary(); 
