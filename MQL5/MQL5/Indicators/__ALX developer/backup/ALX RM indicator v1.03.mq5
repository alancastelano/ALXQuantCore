...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(id == CHARTEVENT_CUSTOM + 1001) { ObjectsDeleteAll(0, g_prefix); ChartRedraw(); }
}
...
/*
void OnDeinit(const int reason) { Painel.Destroy(reason); CleanAllObjects(); }

void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   Painel.ChartEvent(id, lparam, dparam, sparam);
   if(id == CHARTEVENT_CUSTOM + 1001) {
      CleanAllObjects(); ResetGlobals(); ResetStats(); g_firstRun = true; ChartRedraw();
   }
}
*/
