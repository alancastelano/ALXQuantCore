"""
ALXQuant Alpha Research Engine v3.0
====================================
   Gera relatório PDF profissional com análise quantitativa institucional.
   Correções: encoding utf-8-sig, ordem de execução, regime index, MQL5 code gen,
              callbacks PDF via closures.
...
COLUMNS = [
    ...
    'InitialRisk', 'ResultR', 'ResultPercent', 'ProfitFactorTrade',
...
assert len(COLUMNS) == 107, f"Expected 107 columns, got {len(COLUMNS)}"
...
def load_and_clean_data(file_path):
    ...
    - Remove replace de virgula (CSV usa ponto decimal agora)
    - Detecta e pula linha de header se presente
    ...
    for enc in encodings:
        try:
            with p.open('r', encoding=enc, newline='') as f:
                ...
                rows = list(reader)
            logger.info(f"Arquivo lido com encoding {enc}: {len(rows)} linhas brutas")
            break
    ...
    rows = [r for r in rows if len(r) == 107]
    filtered = before_filter - len(rows)
    if filtered > 0:
        logger.info(f"[CLEAN] {filtered} linhas ignoradas (colunas != 107)")

    if not rows:
        raise ValueError("Nenhuma linha valida (107 colunas) encontrada no CSV")
    ...
    logger.info(f"[DONE] {len(df)} trades carregados e limpos")
    return df
...
class QuantAnalyzer:
    ...
    def _create_derived_columns(self):
        ...
        # Calcular Z-Scores a partir de valores raw (v5 CSV)
        for raw_col, z_col, state_name, labels in [
            ...
            if raw_col in self.df.columns and raw_col not in ['', '']:
                self.df[z_col] = (self.df[raw_col] - self.df[raw_col].mean()) / self.df[raw_col].std()
    ...
    def _basic_stats(self):
        ...
        dd = pnl_cum - running_max
        max_dd_amt = dd.min()
        max_dd_pct = max_dd_amt / max(abs(pnl_cum.iloc[0]), 0.01) * 100 if len(pnl_cum) > 0 else 0
        calmar = abs(sharpe_annual / max(max_dd_pct / 100, 0.01)) if max_dd_pct < 0 else 0
    ...
    def _tail_risk(self):
        ...
        pnl_cum = self.df['NetProfit'].cumsum()
        running_max = pnl_cum.cummax()
        dd = pnl_cum - running_max
        self.results['tail_risk'] = {
            ...
            'max_drawdown_pct_pct': dd.min() / max(abs(pnl_cum.iloc[0]), 0.01) * 100,
        }
    ...
    def _load_macro_csv(self):
        ...
        macro_path = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                                  'data', 'risk_sentiment_daily.csv')
...
def load_mql5_report(file_path=r'C:\ALXQuant\data\ReportTester-61542020.html'):
...
class PDFBuilder:
    ...
    def build_pdf(self):
        ...
        # Page 11: Correlation
        self._add(PageBreak())
        self._section(10, "Matriz de Correlacao")
...
def main():
    ...
    if not output_path and args.csv:
        ...
        # Remove '_miner' suffix, add '_report'
        base = base.replace('_miner', '')
        output_path = os.path.join(
            r'C:\ALXQuant\data\report',
            f'{base}_report.pdf'
        )
    elif not output_path:
        output_path = r'C:\ALXQuant\data\report\alpha_research_report.pdf'
