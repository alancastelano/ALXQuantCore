"""
ALXQuant Asset DNA Profiler v7.0-Full
======================================
INTEGRACAO COMPLETA:
[OK] Matematica institucional: DFA, FFD, Kalman, KSG, CPCV, Surrogate (v6.0)
[OK] PBO verdadeiro (Bailey & Lopez de Prado) + Optimal Stopping Walk-Forward (v6.1)
[OK] PDF completo 10 secoes + tabelas PBO/Optimal (v2.1 + v6.1)
[OK] JSON superset com ea_trading_parameters e mql5_directives
[OK] 5 regimes: TREND_FORTE, TREND_FRACO, BREAKOUT, RANGE, CHOP (v7.0)
[OK] Batch multi-ativo via Joblib Parallel (v2.1)
[OK] Significance AAFT phase randomization + Block Bootstrap (v6.1)
...
import config
import duckdb
DB_PATH = config.DB_PATH # Caminho do seu banco
import numpy as np
import pandas as pd
...
def _connect_db(retries=5, retry_delay=1.0):
    """Abre DuckDB com retry para tratar lock ('sendo usado')."""
    last_err = None
...
_DATA_DIR = config.PROJECT_ROOT / "data" / "datasets"

if HAS_PYDANTIC:
    class Config(BaseSettings):
        output_dir: Path = config.PROJECT_ROOT / "data" / "datasets"
        report_dir: Path = config.PROJECT_ROOT / "data" / "mql5"
        cache_dir: Path = config.PROJECT_ROOT / "data" / "cache" / "asset_dna"
...
def _hurst_significance_test(log_prices: np.ndarray, window: int,
    ...
    surrogate_hursts = []
    surrogates = _phase_randomization_surrogate(returns, n_surrogates)
    for sr in surrogates:
        log_prices_surr = np.cumsum(np.concatenate([[log_prices[0]], sr]))
...
class DataLoader:
    ...
    def _load_from_db(self) -> pd.DataFrame:
        years = getattr(self, 'years', 5) # Pega o valor padrao se nao existir
        logger.info(f"[LOAD] DuckDB: {self.symbol}_{self.tf} (Últimos {years} anos)")
        
        # Conecta diretamente no DuckDB sem depender de outros arquivos
        conn = _connect_db()
        try:
...
class FeatureEngine:
    ...
    def _compute_polars(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # [FIX] O DFA deve ser calculado sobre LOG RETURNS, nao sobre LOG PRICES absolutos!
        # Log prices nao sao estacionarios, o que infla o Hurst para ~1.0.
    ...
    def _compute_pandas(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if hurst_cached is not None:
            ...
            logger.info(f"  [DFA] Calculando (sobre Log Returns)...")
            t_h = time.time()
            hurst = _dfa_numba(log_ret_arr, hurst_win)
            logger.info(f"  [DFA] OK em {time.time()-t_h:.2f}s")
            CACHE.put("dfa_hurst_v5", log_ret_arr.astype(np.float32), hurst)
...
class HMMRegimeModel:
    ...
    def _build_features(self, df: pd.DataFrame, window: int = 60) -> np.ndarray:
        ...
        # [FIX] Skew e Kurt vetorizados via Pandas (muito mais rapido)
        s = pd.Series(log_ret)
        skew = s.rolling(window).skew().to_numpy()
        kurt = s.rolling(window).kurt().to_numpy()
        
        # [FIX] Autocorrelacao vetorizada ( substitui o loop np.corrcoef que travava o PC )
    ...
    def classify(self) -> pd.DataFrame:
        ...
        logger.info(f"  [HMM-CAUSAL] Walk-forward: {len(wf_steps)} iteracoes (stride={stride})")
...
class TemporalProfiler:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        df = self.df.copy() # Evita SettingWithCopyWarning
        
        # [FIX] Garante que as colunas de tempo existam, criando-as a partir do indice se necessario
        ...
        # Garante que as colunas sejam numericas para o groupby
...
class MacroRiskIntegrator:
    ...
    PRIORITY_MACRO_DRIVERS = {
        'EURUSD': ['DTWEXBGS', 'DGS2', 'DGS10', 'T10Y2Y', 'DTWEXEMEGS', 'VIX'],
        'GBPUSD': ['DTWEXBGS', 'DGS2', 'DGS10', 'VIX'],
        'USDJPY': ['DTWEXBGS', 'DGS10', 'T10Y2Y', 'VIX'],
        'XAUUSD': ['DTWEXBGS', 'DGS10', 'T10Y2Y', 'VIX', 'CPIAUCSL'],
        '_DEFAULT': ['DTWEXBGS', 'VIX', 'DGS10'],
    }

    def _get_priority_drivers(self, symbol: str) -> list:
        return self.PRIORITY_MACRO_DRIVERS.get(symbol, self.PRIORITY_MACRO_DRIVERS['_DEFAULT'])
    ...
    def cross_reference(self, asset_df: pd.DataFrame, symbol: str = "") -> Dict[str, Any]:
        ...
        # Priority drivers por ativo (para EA MQL5)
        priority_syms = self._get_priority_drivers(symbol)
        priority_drivers = {}
        for ps in priority_syms:
            if ps in exclude_self or ps not in merged.columns:
                continue
            valid_direct = merged[['close_ret', ps]].dropna()
            if len(valid_direct) < 20:
                continue
            r_dir, p_dir = stats.spearmanr(valid_direct['close_ret'], valid_direct[ps])
            best_lag = 0
            best_corr = 0.0
            best_p = 1.0
            for lag in [5, 10, 20]:
                if len(valid_direct) <= lag:
                    continue
                r_lag, p_lag = stats.spearmanr(
                    valid_direct[ps].iloc[:-lag],
                    valid_direct['close_ret'].iloc[lag:]
                )
                if abs(r_lag) > abs(best_corr):
                    best_lag = lag
                    best_corr = r_lag
                    best_p = p_lag
            info = self.catalog.get(ps, {'name': ps, 'category': '', 'country': ''})
            priority_drivers[ps] = {
                'name': info['name'],
                'category': info['category'],
                'country': info['country'],
                'corr_close_ret': round(r_dir, 4),
                'p_close_ret': round(p_dir, 4),
                'best_lag_days': best_lag,
                'best_lag_corr': round(best_corr, 4),
                'best_lag_p': round(best_p, 4),
            }

        # [FIX-4] Leading indicators COM FDR Benjamini-Hochberg
        leading: List[Dict] = []
        all_tests: List[Dict] = []
        scan_universe = list(dict.fromkeys(priority_syms + macro_symbols))[:40]
        for ms in scan_universe:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'methodology_note': (
                ...
                'NÃO e o Probability of Backtest Overfitting (PBO) de Bailey & Lopez de Prado, '
...
class InstitutionalValidators:
    ...
    def _ensure_datetime_index(df: pd.DataFrame) -> pd.DataFrame:
        """Garante que o DataFrame tenha DatetimeIndex para resample."""
...
# ==============================================================
# CHART GENERATOR (13+ graficos do v2.1 preservados)
# ==============================================================
...
# ==============================================================
# PDF REPORT BUILDER (todas as 10 secoes do v2.1 preservadas)
# ==============================================================
class PDFReportBuilder:
    ...
    def build_macro_risk(self):
        ...
        if not charts_avail:
            ...
            self._add(PageBreak()); return
        for key, title in [('macro_corr_heatmap', 'Top Correlacoes Macro vs Ativo'),
    ...
    def build_pdf(self):
        ...
        doc = SimpleDocTemplate(
            ...
            title="ALXQuant Asset DNA Profile v7.0",
    ...
    def _page_cover(canvas_obj, doc):
        ...
        canvas_obj.drawString(35*mm, h/2+5*mm, "Perfil Estrutural Institucional v7.0")
        ...
        canvas_obj.drawString(35*mm, 25*mm, "ALXQuant Asset DNA Profiler v7.0-Full")
        canvas_obj.setFillColor(colors.HexColor(C['gold']))
    ...
    def _page_content(canvas_obj, doc):
        ...
        canvas_obj.drawString(18*mm, h-14*mm, "ALXQUANT  |  ASSET DNA v7.0")
        ...
        canvas_obj.drawRightString(w-18*mm, 10*mm, "ALXQuant v7.0")
        canvas_obj.restoreState()
...
class JsonProfileExporter:
    ...
    def _build(self, symbol: str, tf: str) -> dict:
        ...
        priority_drivers = macro_sens.get('priority_drivers', {})
        top_corr = macro_sens.get('top_correlations', [])

        # Extrair do priority_drivers (novo)
        macro_corr_dxy = priority_drivers.get('DTWEXBGS', {}).get('corr_close_ret')
        macro_corr_vix = priority_drivers.get('VIX', {}).get('corr_close_ret')

        # Manter fallback de top_corr para DXY (legado)
        if macro_corr_dxy is None:
            for cr in top_corr:
        ...
        # PBO -> allow_mean_reversion
        allow_mean_reversion = self.pbo.get('mean_reversion_pbo', 100) < 50.0
...
def process_single_asset(symbol: str, tf: str, from_db: bool, years: int,
    ...
    # Garante DatetimeIndex (o pipeline Polars perde o indice temporal)
    if not isinstance(df.index, pd.DatetimeIndex):
        if 'time' in df.columns:
            df = df.set_index('time')
        elif 'datetime' in df.columns:
    ...
    logger.info(f"[OK] Tempo decorrido para gerar o relatorio: {elapsed_str} | Candles: {len(df):,}")
...
def main():
    parser = argparse.ArgumentParser(description='ALXQuant Asset DNA Profiler v7.0-Full')
    ...
    logger.info(f"Historico: {args.years} anos | PDF: {'NÃO' if args.skip_pdf else 'SIM'} | "
                f"Institucional: {'NÃO' if args.skip_institutional else 'SIM'}")
    logger.info("=" * 60)
